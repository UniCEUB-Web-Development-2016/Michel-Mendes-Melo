...
{
	private $requiredParameters = array('name', 'last_name', 'cpf', 'rg', 'birthdate', 'phone', 'addres', 'category', 'salary');
	...
	{
		$query =  "INSERT INTO employee (name, last_name, cpf, rg, birthdate, phone, addres, category, salary) VALUES ('".$employee->getName()."','".
	...
	public function update($request)
	{
		if(!empty($_GET["id"]) && !empty($_GET["name"]) && !empty($_GET["last_name"]) && !empty($_GET["cpf"]) && !empty($_GET["rg"]) && !empty($_GET["birthdate"])
							&& !empty($_GET["phone"])&& !empty($_GET["addres"])  && !empty($_GET["category"]) && !empty($_GET["salary"])) {
			...
			$result = $conn->prepare("UPDATE employee SET name=:name, last_name=:last_name, cpf=:cpf, rg=:rg, birthdate=:birthdate,  
									phone=:phone, addres=:addres, category=:category, salary=:salary WHERE id=:id");
